...
export default function RepairUpdatePage() {
  ...
  const updateStatus = async () => {
    ...
    // ✅ 3. Validation ก่อน submit: บังคับ Final Price ตอน Close
    if (Number(newStatus) === CLOSE_STATUS_ID && (!finalPrice || finalPrice <= 0)) {
      return Swal.fire("กรุณาใส่ราคาปิดงาน", "ต้องระบุราคาที่ถูกต้องก่อนปิดงานซ่อม", "warning");
    }
    ...
    try {
      // ✅ 4. ส่ง final_price ไป backend (ส่งเฉพาะเมื่อมีการปิดงาน)
      const payload = {
        status_id: newStatus,
        note: note,
      };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col h-[70vh] items-center justify-center gap-4 bg-gray-50/50">
        ...
        <p className="font-bold text-gray-500 animate-pulse tracking-wide uppercase text-xs">Loading...</p>
      </div>
    );
  }

  if (!repair) {
    return <div className="p-10 text-center text-red-500 font-bold">ไม่พบข้อมูลงานซ่อม</div>;
  }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border ${repair.status?.id === 4 ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-blue-50 text-blue-600 border-blue-100"}`}>
        ...
        <div className="lg:col-span-2 space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-gray-50 shadow-sm">
                    ...
                    <p className="font-black text-xl text-gray-800 mb-1">{repair.device?.brand} {repair.device?.model}</p>
            ...
            <div className="bg-white p-8 rounded-[2rem] border border-gray-50 shadow-sm">
                <h2 className="font-black text-gray-800 mb-8 flex items-center gap-2">
                    <History size={20} className="text-blue-600" /> ประวัติการดำเนินการ
                </h2>
                <div className="relative ml-4 space-y-8 before:absolute before:inset-0 before:ml-1.5 before:w-0.5 before:bg-gray-100 before:h-full">
                    {repair.timelines?.slice().reverse().map((t, idx) => (
                        <div key={t.id} className="relative pl-10 group">
                            <div className={`absolute left-0 top-1 w-4 h-4 rounded-full ring-4 ring-white transition-all ${idx === 0 ? "bg-blue-600 scale-125" : "bg-gray-300"}`}></div>
                            ...
                            {t.note && <div className="text-xs text-gray-600 bg-gray-50/80 p-3 rounded-xl border border-gray-100 font-medium">{t.note}</div>}
                        </div>
                    ))}
                </div>
            </div>
          ...
          <div className="bg-gray-900 p-8 rounded-[2.5rem] shadow-xl shadow-blue-100 space-y-6 sticky top-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <select
                  className="w-full bg-gray-800 border-none text-white p-4 rounded-2xl outline-none focus:ring-2 ring-blue-500 transition-all font-bold"
                  value={newStatus}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-blue-300 uppercase tracking-widest ml-1">โน้ตแจ้งลูกค้า</label>
                <textarea
                  className="w-full bg-gray-800 border-none text-white p-4 rounded-2xl outline-none focus:ring-2 ring-blue-500 transition-all text-sm min-h-[100px]"
              ...
              {/* ✅ 2. Input final price ตอน status = 5 (จัด Format ให้เข้ากับธีม) */}
              {Number(newStatus) === CLOSE_STATUS_ID && (
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                  <label className="text-[10px] font-black text-emerald-400 uppercase tracking-widest ml-1 flex items-center gap-1">
                    <CircleDollarSign size={12} /> ราคาปิดงาน (Final Price)
                  </label>
                  <div className="relative">
                    <input
                      type="number"
                      ...
                      placeholder="0.00"
                      className="w-full bg-gray-800 border-2 border-emerald-500/30 text-emerald-400 p-4 rounded-2xl outline-none focus:ring-2 ring-emerald-500 font-black text-lg pr-12 transition-all"
                      disabled={isUpdating}
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-600 font-bold text-sm">THB</span>
                  </div>
                </div>
              )}
