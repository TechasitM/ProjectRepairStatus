      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-t-xl shadow-sm p-6 border-b border-gray-100">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-gray-500 text-sm">รหัสงาน: {repair?.repair_code}</p>
            </div>
        ...
        <div className="bg-white p-6 shadow-sm space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-gray-400 uppercase">อาการเสีย</label>
              <p className="text-gray-700">{repair?.problem_description}</p>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-400 uppercase">วันที่รับเครื่อง</label>
              <p className="text-gray-700">
                {repair?.receive_date ? new Date(repair.receive_date).toLocaleDateString("th-TH") : "-"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white mt-4 rounded-b-xl shadow-sm p-6">
          <h2 className="text-lg font-bold text-gray-800 mb-6">ประวัติการดำเนินการ</h2>
            ...
            {repair?.timelines?.length > 0 ? (
              repair.timelines.map((item, index) => (
                <div key={item.id} className="relative pl-8 pb-8 last:pb-0">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-800">{item.status?.status_name}</h3>
              ...
              <p className="text-gray-400 text-center py-4">ยังไม่มีประวัติสถานะ</p>
            )}
          </div>
        </div>
